...
class Model():
    ...
    def build(self, sample_dict):

        compiled_eq = OrderedDict((eqkey, eqval.equation) for 
                eqkey, eqval in self.listOfEquations.items())
        ...
        def fun(t, y):
            def f(self, t, y):
                ...
                for i, var in enumerate(self.listOfVariables):
                    curr_state[var] = y[i]
    ...
    def run_as_ODE(self, solver='RK45'):
        # this is for testing purpose only
        ics = [var.initial_condition.value for var \
                in self.listOfVariables.values()]
        t_span = [self.sim_time[0], self.sim_time[-1]]
        sol = solve_ivp(self.func, t_span , ics, method=solver, 
                t_eval=self.sim_time)
        return sol
...
class Patient():
    ...
    def __init__(self, ID, model_init_args, 
            sample_dict, outcome=None):
        
        self.ID = ID
        self.patient_params = sample_dict
        self.outcome = outcome

        # Build model to create unique patient
        self.model = Model(*model_init_args)
        self.model.add_Parameter_Values(sample_dict)
        self.model.build(sample_dict)
    ...
    def check_outcome():
        # Use outcome functions to test against the data
        pass
...
class Simulation():
    ...
    def create_patients(self, modelargs, 
        ...
        for i, sample in enumerate(sample_list):
            for j, param in enumerate(sample):
                arr = np.array([p_dict[x] for x in sample])
                patient_arr[num_patients*j:num_patients
                    +num_patients*j ,slice_index[j]] = arr.T
